...
const WriteDiary = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const newEntry = {
      ...
      spotifyTrackId: input.selectedSong.id,
      trackName: input.selectedSong.name,
      artistName: input.selectedSong.artists.map(artist => artist.name).join(", "),
      albumImageUrl: input.selectedSong.album.images[0]?.url,
      previewUrl: input.selectedSong.preview_url
    };

    try {
      await axios.post('http://localhost:3080/api/posts', newEntry);
                    ...
                    <div key={index} className="flex justify-between items-center mb-4 last:mb-0">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{song.artists.map(artist => artist.name).join(", ")}</p>
                        </div>
                      </div>
                ...
                <div className="mb-4 p-4 bg-green-100 rounded-lg">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{input.selectedSong.artists.map(artist => artist.name).join(", ")}</p>
                    </div>
                  </div>
